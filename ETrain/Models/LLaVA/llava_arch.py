...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        global_text_feature = torch.stack([param for param in self.global_text_feature[:self.expert_num]])
        global_image_feature = torch.stack([param for param in self.global_image_feature[:self.expert_num]]) 
        ...
        max_sim = cos_sim[max_idx].item()

        # maha_dis = euclidean_distance(text_guide_features_mean, global_text_feature)
        # print(maha_dis)

        if self.training:
            self.text_features_dict[self.client_idx].extend(text_guide_features)
            self.image_features_dict[self.client_idx].extend(image_guide_features)

            if any(param.data.sum() != 0 for param in self.cluster_text_size) and self.lora_id_signal != True:
                if max_sim > 0.9:
                    self.lora_id = max_idx
                else:
                    zero_idx = next((i for i, param in enumerate(self.cluster_text_size) if param.data.sum() == 0), -1)
                    if zero_idx != -1:
                        self.lora_id = zero_idx
                    else:
                        self.lora_id = max_idx
            num_layers = 32
            proj_names = [
            ...
            for layer_idx in range(num_layers):
                for proj_name in proj_names:
                    ...
                    proj_layer.lora_id = self.lora_id
        else:
