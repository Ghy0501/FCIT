        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if not param.requires_grad:
                    ...
                    device = param.device
                    self.global_state[name] = self.global_state[name].to(device)
                    auxiliary_new_para[name] = (self.global_state[name] - param) / (10 * self.args.learning_rate) - self.correction[name]
